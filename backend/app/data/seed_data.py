...
class DemoStore:
    ...
    def __init__(self) -> None:
        ...
        self.procurement_statuses: dict[str, str] = {}
        self.redistribution_statuses: dict[str, str] = {}
    ...
    def approve_procurement(self, recommendation_id: str, approved: bool) -> str:
        status = "approved" if approved else "rejected"
        self.procurement_statuses[recommendation_id] = status
        return status

    def approve_redistribution(self, opportunity_id: str, ingredient_id: str, from_outlet_id: str, to_outlet_id: str, quantity: float, approved: bool) -> str:
        status = "approved" if approved else "rejected"
        if status == "approved" and self.redistribution_statuses.get(opportunity_id) != "approved":
            remaining = quantity
            for index, batch in enumerate(self.list_batches(from_outlet_id)):
                if batch.ingredient_id != ingredient_id or remaining <= 0:
                    continue
                moved = min(batch.quantity, remaining)
                remaining = round(remaining - moved, 2)
                original_index = self.inventory_batches.index(batch)
                self.inventory_batches[original_index] = replace(batch, quantity=round(batch.quantity - moved, 2))

            source_batch = next(
                (batch for batch in self.inventory_batches if batch.outlet_id == from_outlet_id and batch.ingredient_id == ingredient_id),
                None,
            )
            ingredient = self.ingredient(ingredient_id)
            self.inventory_batches.append(
                InventoryBatch(
                    id=f"b-transfer-{len(self.inventory_batches) + 1}",
                    outlet_id=to_outlet_id,
                    ingredient_id=ingredient_id,
                    batch_code=f"TR-{opportunity_id}",
                    quantity=round(quantity - remaining, 2),
                    unit=ingredient.unit,
                    purchase_date=REFERENCE_DATE,
                    expiry_date=source_batch.expiry_date if source_batch else REFERENCE_DATE + timedelta(days=ingredient.default_shelf_life_days),
                    supplier_id=source_batch.supplier_id if source_batch else self.suppliers[0].id,
                    storage_condition=source_batch.storage_condition if source_batch else "normal",
                )
            )
        self.redistribution_statuses[opportunity_id] = status
        return status

...
## 1. Product Overview
FoodWise AI is a decision-support platform for food-service businesses. It ingests sales, inventory, and waste data, predicts demand and spoilage risk, and recommends concrete actions — what to prepare, what to buy, and what to move between outlets before it becomes waste. The long-term goal is to close the loop by comparing predictions to actual outcomes and using that to improve future recommendations; that feedback loop is future scope (see Section 13).
...
Cares about waste cost and patterns across the whole chain — which outlet wastes the most, and whether interventions are actually working.
...
- Eventually build a system that gets better over time by comparing its predictions to what actually happened (planned — not yet built, see `07_AI_ML_DESIGN.md`)
...
- FR1: System shall forecast demand per ingredient per outlet — **built**
- FR2: System shall track inventory with batch, expiry, and shelf-life data — **built**
- FR3: System shall compute a spoilage risk score (Low/Medium/High) per inventory batch — **built**
- FR4: System shall flag storage/quality anomalies for human review only — **built** (risk-based alerts)
- FR5: System shall detect recurring supplier/batch patterns across outlets — **planned, not built**
- FR6: System shall recommend inter-outlet redistribution when surplus and shortage coexist — **built**
- FR7: System shall recommend procurement quantities and timing — **built**
- FR8: System shall recommend preparation quantities with a reasonable buffer — **built**
- FR9: System shall generate alerts for risk — **built**; overstock/understock/opportunity alert types are **planned**
- FR10: System shall provide waste analytics (by ingredient, outlet, reason) — **built**; cost trend over time is **planned**
- FR11: System shall generate a root-cause explanation for waste patterns — **built** (pattern detection + template-based summary; LLM phrasing is planned)
- FR12: System shall compare predicted vs actual outcomes to refine future recommendations — **planned, not built**

## 9. Feature Requirements
See `01_PROJECT_OVERVIEW.md` for the full feature list, split into built vs. planned. Each built feature maps to a corresponding API group in `06_API_SPECIFICATION.md` and a data entity set in `05_DATABASE_DESIGN.md`.
...
3. Staff logs a waste event → system updates waste analytics and the root-cause view.
...
## 12. MVP Scope
- Single organization, 3 demo outlets
- Seeded in-memory data (no live POS integration, no database yet — see `05_DATABASE_DESIGN.md`)
...
- Root-cause pattern detection, with a grounded (template-based, not yet LLM-generated) explanation
...
- A persistent database (PostgreSQL) behind the API
- A working Gemini/LLM integration for explanation text
- Supplier/batch pattern detection
- Prediction-vs-actual accuracy tracking (the continuous learning loop)
...
- Forecast accuracy (MAPE/WAPE) per item/time-window — requires the accuracy-tracking work in Section 13
...
- No accusatory language toward suppliers, once supplier pattern detection is built — pattern detection only, with "review recommended" phrasing
